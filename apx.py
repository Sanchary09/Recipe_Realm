...
class Recipe(Base):
    ...
    category = Column(String)  # Add category to differentiate vegetarian/non-vegetarian
...
The Recipe Recommendation Application is an innovative platform that enhances the culinary experience by enabling users to manage, discover, and explore a diverse range of recipes. The application allows users to easily add, update, and delete personalized recipes while categorizing them as vegetarian or non-vegetarian. 
""")
...
# Manage Recipes page
    ...
    if recipes:
        ...
        with st.form(key="add_recipe_form"):
            ...
            category = st.selectbox("Category", ["Vegetarian", "Non-Vegetarian"])
...
# Recipe Ideas page
    ...
    if st.button("Search"):
        if dish_name:
            # Search for YouTube videos related to the dish name entered by the user.
            youtube_results = search_youtube(dish_name)

            if youtube_results:
                for video in youtube_results:
                    video_title = video['snippet']['title']+"recipe"
                    video_id = video['id']['videoId']
                    video_url = f"https://www.youtube.com/watch?v={video_id}"
                    thumbnail_url = video['snippet']['thumbnails']['medium']['url']
                    ...
                    col1, col2 = st.columns(2)
                    with col1:
                        # Only display cooking-related videos based on user input.
                        if "cooking" in video_title.lower() or dish_name.lower() in video_title.lower():
                            col1.image(thumbnail_url)  # Display thumbnail with fixed width
                            col2.markdown(f"[{video_title}]({video_url})")
                            continue  # Skip non-cooking videos.

                else:
                    # If no cooking videos found based on dish name search,
                    # let's also check if there's a matching recipe in our database.
                    ...
                    if matching_recipes:
                        for row in matching_recipes:
                            ...
                            col1, col2 = st.columns(2)
                            with col1:
                                # display each matching recipe's details.
